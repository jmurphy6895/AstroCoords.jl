...
Delauney(L::LT, G::GT, H::HT, M::MT, ω::OT, Ω::OmT) where {LT, GT, HT, MT, OT, OmT} = Delauney{promote_type(LT,GT,HT,MT,OT,Omt)}(L, G, H, M, ω, Ω)
...
function Base.getindex(p::Delauney, i::Int)
    if i == 1 return p.L
    elseif i == 2 return p.G
    elseif i == 3 return p.H
    elseif i == 4 return p.M
    elseif i == 5 return p.ω
    elseif i == 6 return p.Ω
