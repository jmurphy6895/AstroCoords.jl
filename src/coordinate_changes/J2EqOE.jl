...
@inline function _step5(aⱼ₂::Number, γ::Number, γ′::Number, θ::Number, rⱼ₂::Number, η::Number, eⱼ₂::Number, gⱼ₂::Number, νⱼ₂::Number, Lⱼ₂::Number)
    a = aⱼ₂ * (1 + γ*((-1 + 3*θ^2) * ((aⱼ₂^3) / (rⱼ₂^3) - η^(-3)) + 3 * (1 - θ^2) * (aⱼ₂^3) / (rⱼ₂^3) * cos(2*gⱼ₂ + 2*νⱼ₂)))
    δh = -.5 * γ′ * θ * (6 * (νⱼ₂ - Lⱼ₂ + eⱼ₂ * sin(νⱼ₂)) - 3 * sin(2*gⱼ₂ + 2*νⱼ₂) - 3*eⱼ₂*sin(2*gⱼ₂ + νⱼ₂) - eⱼ₂*sin(2*gⱼ₂ + 3*νⱼ₂)) 

    return a, δh
end

@inline function _step6(γ′::Number, θ::Number, νⱼ₂::Number, Lⱼ₂::Number, eⱼ₂::Number, gⱼ₂::Number, hⱼ₂::Number, δh::Number)
    Σlgh = .25 * γ′ * (6*(-1 + 5*θ^2) * (νⱼ₂ - Lⱼ₂ + eⱼ₂*sin(νⱼ₂)) + (3 - 5*θ^2) * (3*sin(2*gⱼ₂ + 2*νⱼ₂) + 3*eⱼ₂*sin(2*gⱼ₂ + νⱼ₂) + eⱼ₂*sin(2*gⱼ₂ + 3*νⱼ₂))) + δh + Lⱼ₂ + gⱼ₂ + hⱼ₂
    
    return Σlgh
end

@inline function _step7(νⱼ₂::Number, eⱼ₂::Number, η::Number, aⱼ₂::Number, rⱼ₂::Number, γ::Number, γ′::Number, θ::Number, gⱼ₂::Number)
    v1 = 3*cos(νⱼ₂) + 3*eⱼ₂*cos(νⱼ₂)^2 + eⱼ₂^2*cos(νⱼ₂)^3
    v2 = η^(-6) * (eⱼ₂*η + eⱼ₂ / (1 + η) + v1)
    v3 = η^(-6) * (eⱼ₂ + v1)
    v4 = ((aⱼ₂ * η) / rⱼ₂)^2 + aⱼ₂ / rⱼ₂

    δe = .5 * η^2 * (γ * ((-1 + 3*θ^2)*v2 + 3*(1 - θ^2)*v3*cos(2*gⱼ₂ + 2*νⱼ₂)) - γ′*(1-θ^2)*(3*cos(2*gⱼ₂ + νⱼ₂) + cos(2*gⱼ₂ + 3*νⱼ₂)))
    e″δL = -.25 * η^3 * γ′ * (2*(-1 + 3*θ^2) * (v4+1) * sin(νⱼ₂) + 3*(1-θ^2) * ((-v4 + 1)*sin(2*gⱼ₂ + νⱼ₂) + (v4 + 1/3)*sin(2*gⱼ₂ + 3*νⱼ₂)))

    return v1, v2, v3, v4, δe, e″δL
end

@inline function _step8(γ′::Number, θ::Number, νⱼ₂::Number, eⱼ₂::Number, gⱼ₂::Number, δh::Number, Iⱼ₂::Number)
    δI = .5 * γ′ * θ * √(1 - θ^2) * (3*cos(2*gⱼ₂ + 2*νⱼ₂) + 3*eⱼ₂*cos(2*gⱼ₂ + νⱼ₂) + eⱼ₂*cos(2*gⱼ₂ + 3*νⱼ₂))
    sin_half_I″_δh = (sin(Iⱼ₂) * δh) / (2*cos(.5 * Iⱼ₂))

    return δI, sin_half_I″_δh
end

@inline function _step9(a::Number, eⱼ₂::Number, δe::Number, e″δL::Number, Lⱼ₂::Number, Iⱼ₂::Number, δI::Number, sin_half_I″_δh::Number, hⱼ₂::Number, Σlgh::Number)
    I1 = a
    I2 = (eⱼ₂ + δe) * sin(Lⱼ₂) + e″δL * cos(Lⱼ₂)
    I3 = (eⱼ₂ + δe) * cos(Lⱼ₂) - e″δL * sin(Lⱼ₂)
    I4 = (sin(.5 * Iⱼ₂) + cos(.5 * Iⱼ₂) * .5 * δI) * sin(hⱼ₂) + sin_half_I″_δh * cos(hⱼ₂)
    I5 = (sin(.5 * Iⱼ₂) + cos(.5 * Iⱼ₂) * .5 * δI) * cos(hⱼ₂) - sin_half_I″_δh * sin(hⱼ₂)
    I6 = Σlgh
...
-`u_IOE::SVector{6, <:Number}`: The Intermediate Orbit Element vector [I1; I2; I3; I4; I5; I6].
"""
function J2IOE2IOE(u::AbstractVector{T}, μ::V; J2::W=1.0826261738522e-03, Req::Z=6.378137e+03) where {T<:Number, V<:Number, W<:Number, Z<:Number}
...
"""
function IOE2J2IOE(u::AbstractVector{T}, μ::V; Req::W=6.378137e+03, tol::AbstractFloat=10*eps(promote_type(T, V)), max_iter::Int=100) where {T<:Number, V<:Number, W<:Number}
