...
struct Spherical{T} <: AstroCoord{6, T}
    ...
    θdot::T
    ϕdot::T
    @inline Spherical{T}(r, θ, ϕ, ṙ, θdot, ϕdot) where T = new{T}(r, θ, ϕ, ṙ, θdot, ϕdot)
    @inline Spherical{T}(X::Spherical) where T = new{T}(X.r, X.θ, X.ϕ, X.ṙ, X.θdot, X.ϕdot)
end

# ~~~~~~~~~~~~~~~ Constructors ~~~~~~~~~~~~~~~ #
Spherical(X::AbstractArray{T, 1}) where T = Spherical{T}(X...)
Spherical(r::R, θ::T, ϕ::P, ṙ::RD, θdot::TD, ϕdot::PD) where{R,T,P,RD,TD,PD} = Spherical{promote_type(R,T,P,RD,TD,PD)}(r, θ, ϕ, ṙ, θdot, ϕdot)
(::Type{S})(g::StaticVector) where S<:Spherical = Spherical(g[1], g[2], g[3], g[4], g[5], g[6])
...
function Base.getindex(p::Spherical, i::Int)
    if i == 1 return p.r
    elseif i == 2 return p.θ
    elseif i == 3 return p.ϕ
    elseif i == 4 return p.ṙ
    elseif i == 5 return p.θdot
    elseif i == 6 return p.ϕdot
...
struct Cylindrical{T} <: AstroCoord{6, T}
    ...
    @inline Cylindrical{T}(X::Cylindrical) where T = new{T}(X.r, X.θ, X.z, X.ṙ, X.θdot, X.ż)
end

# ~~~~~~~~~~~~~~~ Constructors ~~~~~~~~~~~~~~~ #
Cylindrical(X::AbstractArray{T, 1}) where T = Cylindrical{T}(X...)
Cylindrical(r::R, θ::T, z::Z, ṙ::RD, θdot::TD, ż::ZD) where{R,T,Z,RD,TD,ZD} = Cylindrical{promote_type(R,T,P,RD,TD,ZD)}(r, θ, z, ṙ, θdot, ż)
(::Type{C})(g::StaticVector) where C <: Cylindrical = Cylindrical(g[1], g[2], g[3], g[4], g[5], g[6])
